...
def validateLogRegr( data, sgd ):
     """ 
     TAkes the data Generated by genData1D, and performs 4 logistic regressions
     to test if the results will be the same
     
     """
     
     index0 = np.where( data[:, 3 ] == 0 )[0]
     index1 = np.where( data[:, 3] == 1 )[0]
     
     
     memb0 = np.zeros( len( data ) )
     memb0[index0] = 1
     memb0 = memb0#*( len(memb0)/np.sum(memb0) )
     
     memb1 = np.zeros( len( data ) )
     memb1[ index1 ] = 1
     memb1 = memb1#*( len(memb1)/np.sum(memb1) )
     ...
     alpha = 0.0000000000000000000000000000000001
     C = 100000000000000000000000000
     
     if sgd == 1:
         model = SGDClassifier( loss = 'log', penalty = 'l1', alpha = alpha,
                           random_state = 0 )
     else:
     
         model = LogisticRegression( penalty = 'l1', C = C, random_state = 0)
   
     m0 = model.fit( data0[:,1].reshape(-1, 1), data0[:,2])
     c0 = [m0.coef_.tolist()[0][0], m0.intercept_.tolist()[0]]
#     c0[1] = c0[1]/abs(c0[0])
#     c0[0] = c0[0]/abs(c0[0])
#     
     ...
     m00 = model.fit( data[:,1].reshape(-1, 1), data[:,2], sample_weight = memb0)
     c00 = [m00.coef_.tolist()[0][0], m00.intercept_.tolist()[0]]
#     c00[1] = c00[1]/abs(c00[0])
#     c00[0] = c00[0]/abs(c00[0])
     ...
     m1 = model.fit( data1[:, 1].reshape(-1, 1), data1[:,2] )
     c1 = [m1.coef_.tolist()[0][0], m1.intercept_.tolist()[0]]
     c1[1] = c1[1]/abs(c1[0])
     c1[0] = c1[0]/abs(c1[0])
     ...
     m11 = model.fit( data[:,1].reshape(-1,1), data[:,2], sample_weight =  memb1)
     c11 = [m11.coef_.tolist()[0][0], m11.intercept_.tolist()[0]]
     c11[1] = c11[1]/abs(c11[0])
     c11[0] = c11[0]/abs(c11[0])
     
     
     dictM = {'c0':c0, 'c00':c00, 'c1': c1, 'c11':c11 }
     
     return dictM, memb0, memb1, data0, data1
